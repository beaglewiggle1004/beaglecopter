...
int serialWrite(const uint8_t *packet); 
...
int serialWrite(const uint8_t *packet) {
    ...
    if ((fd = open("/dev/ttyGS0", O_RDWR | O_NOCTTY)) < 0) {
        printf("Could not open ttyGS0.\n");
        return -1;
    }

    if (cfsetospeed(&Serial, B9600) < 0) {
